...
def postprocess(out, conf_th=0.15, nms_th=0.3, img_size=320):
    confs = out[:, 0]
    mask = confs > conf_th
    if not np.any(mask): return []
    filt = out[mask]
    boxes = filt[:, 220:224] / float(img_size)
    keep = nms(boxes, filt[:, 0], nms_th)
    results = []
    for i in keep:
        d = filt[i]
        box = d[220:224] / float(img_size)
...
def main_loop(cam_id, engine_path):
    ...
    while S.running:
        ...
        if len(S.frame_buffer) >= 80 and frame_count % 5 == 0:
            ...
            # Run inference
            out = engine.run(clip)
            dets = postprocess(out, img_size=320)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--engine', default='yowo_multitask_320_fp16_v2.engine')
